...
def is_valid_for_part_one(number):
...
def is_valid_for_part_two(number):
    if not (lower <= number <= upper):
        return False

    as_text = str(number)
    for i in range(0, len(as_text) - 1):
        if as_text[i] == as_text[i+ 1] and as_text.count(as_text[i]) == 2 : return True
            ...
            for n4 in range(n3, 10):
                for n5 in range(n4, 10):
                    for n6 in range(n5, 10):
                        ...
                        if is_valid_for_part_two(toTry):
                            good_passwords += 1
